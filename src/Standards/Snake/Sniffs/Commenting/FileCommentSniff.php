...
class FileCommentSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $tag) {
            ...
            if ($name === '@author') {
                if ($tokens[$string]['content'] !== 'Snake Pty Ltd <products@snake.net>') {
                    $error = 'Expected "Snake Pty Ltd <products@snake.net>" for author tag';
                    $fix   = $phpcsFile->addFixableError($error, $tag, 'IncorrectAuthor');
                    if ($fix === true) {
                        $expected = 'Snake Pty Ltd <products@snake.net>';
                        $phpcsFile->fixer->replaceToken($string, $expected);
                    }
                }
            } else if ($name === '@copyright') {
                if (preg_match('/^([0-9]{4})(-[0-9]{4})? (Snake Pty Ltd \(ABN 77 084 670 600\))$/', $tokens[$string]['content']) === 0) {
                    $error = 'Expected "xxxx-xxxx Snake Pty Ltd (ABN 77 084 670 600)" for copyright declaration';
                    ...
                    if ($fix === true) {
                        ...
                        $expected = $matches[1].' Snake Pty Ltd (ABN 77 084 670 600)';
                        $phpcsFile->fixer->replaceToken($string, $expected);
                    }
                }
            }//end if
